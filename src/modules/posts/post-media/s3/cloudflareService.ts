...
export class CloudflareService {
  ...
  async s3Upload(file, bucket, name, mimetype, FOLDER) {
    ...
    const params = {
      Bucket: bucket,
      Key: `${this.generateRandomString(15)}.` + name.split('.')[1],
      Body: file,
      ACL: 'public-read',
      ContentType: mimetype,
      ContentDisposition: 'inline',
      CreateBucketConfiguration: {
        LocationConstraint: process.env.CLOUDFLARE_DEFAULT_REGION,
      },
      /*Resize: {
                width: 50,
                height: 50,
    ...
    try {
      ...
      const data = {
        Key: s3Response.Location,
        Location: `${process.env.CLOUDFLARE_BUCKET_URL}/${process.env.CLOUDFLARE_BUCKET}/${s3Response.Key}`,
      };
  ...
  async s3Delete(filename) {
    try {
      const res = await this.s3
        .deleteObject({ Bucket: this.CLOUDFLARE_BUCKET, Key: filename })
        .promise();
      return res;
    } catch (error) {
